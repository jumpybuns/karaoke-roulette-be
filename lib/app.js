...
app.get('/api/random-videos', async(req, res) => {
  try {
    const response = await request.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=200&q=karaoke%20singking%20sunfly%20karaoke%20${req.query.search}&type=video&videoEmbeddable=true&key=${process.env.YOUTUBE_API_KEY}`);
