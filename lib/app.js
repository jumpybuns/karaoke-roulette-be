...
app.post('/api/favorites', async(req, res) => {
>>>>>>> 01e51d42a288849b522194bfb30a4cbf38d15962
  try {
    // we add the current user if and the selected movie id to SQL
    const data = await client.query(`
      INSERT into favorites (id, videoId, title, thumbnails, owner_id)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *;
    `, [req.body.id, req.body.videoId, req.body.title, req.body.thumbnails, req.userId]);
...
app.get('/api/random-videos', async (req, res) => {
  try {
    const URL = (`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=100&q=karaoke"%20karaoke%20version"&type=video&videoEmbeddable=true&key=${process.env.YOUTUBE_API_KEY}`);
