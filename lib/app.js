...
app.get('/api/random-videos', async(req, res) => {
  try {
    const response = await request.get(`GET https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=100&q=karaoke%20${req.query.search}(karaoke%20version)&relevanceLanguage=en&safeSearch=none&type=video&videoEmbeddable=true&key=${process.env.YOUTUBE_API_KEY}`);
