...
app.get('/api/random-videos', async(req, res) => {
  try {
    const response = await request.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&channelId=UCwTRjvjVge51X-ILJ4i22ew&maxResults=2000&q=karaoke${req.query.search}&type=video&videoEmbeddable=true&key=${process.env.YOUTUBE_API_KEY}`);
