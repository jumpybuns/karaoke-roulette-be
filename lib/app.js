...
app.get('/api/random-videos', async(req, res) => {
  try {
    const response = await request.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q=karaoke%20sing%20king${req.query.search}&type=video&videoEmbeddable=true&key=${process.env.YOUTUBE_API_KEY}`);
